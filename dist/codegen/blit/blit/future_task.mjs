...
export const FutureTask = {
    ...
    fromAmino(object) {
        return {
            index: object.index,
            scheduled_on: object?.scheduled_on ? fromTimestamp(Timestamp.fromAmino(object.scheduled_on)) : undefined,
            task_id: BigInt(object.task_id),
            status: isSet(object.status) ? futureTaskStatusFromJSON(object.status) : -1,
            gas_price: object?.gas_price ? DecCoin.fromAmino(object.gas_price) : undefined
        };
    },
    toAmino(message) {
        const obj = {};
