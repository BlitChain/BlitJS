...
export const ListenCommitRequest = {
  ...
  fromSDK(object: ListenCommitRequestSDKType): ListenCommitRequest {
    return {
      ...
      change_set: Array.isArray(object?.change_set) ? object.change_set.map((e: any) => StoreKVPair.fromSDK(e)) : []
    };
  },
  toSDK(message: ListenCommitRequest): ListenCommitRequestSDKType {
    ...
    if (message.change_set) {
      obj.change_set = message.change_set.map(e => e ? StoreKVPair.toSDK(e) : undefined);
    } else {
      obj.change_set = [];
    }
    return obj;
